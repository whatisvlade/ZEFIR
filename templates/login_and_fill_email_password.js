...
$(document).ready(function () {
    ...
    monitorLoadingMaskInIframe();
  });
  let IS_SUBMIT_CLICKED = false;
  ...
  // Функция для заполнения email и пароля
  function fillEmailAndPassword() {
    const maxId = 20;
    const possibleEmailIds = Array.from({ length: maxId }, (_, i) => `UserId${i + 1}`);
    const possiblePasswordIds = Array.from({ length: maxId }, (_, i) => `Password${i + 1}`);

    let emailFieldFound = false;
    let passwordFieldFound = false;

    possibleEmailIds.forEach(id => {
        const emailField = $('#' + id);
    ...
    possiblePasswordIds.forEach(id => {
    ...
    if (emailFieldFound && passwordFieldFound) {
        ...
        if (verifyButton.is(':visible')) {
            verifyButton.click();
            console.log('Клик по кнопке Verify выполнен.');

            waitForLoadingMaskToDisappear(() => {
                console.log('Loading завершен.');
                clickMaximizeButton();
                findVisibleDivLikeDevAbilities(); // Добавляем вызов после загрузки
                const captchaIframe = document.querySelector('iframe[title="Verify Registration"]');

                if (!captchaIframe) {
                    alert('Iframe капчи не найден на странице');
                    return;
                }
                analyzeAndSelectCaptchaImages(captchaIframe, true);
            });
        }
  ...
  function waitForSubmitButton() {
    const interval = setInterval(() => {
        ...
        if (loginButton.is(':visible')) {
            loginButton.click();
            console.log('Клик по кнопке Login выполнен.');
            clearInterval(interval);
        }
    }, 1000);
  }

  // Клик по кнопке Maximize
  function clickMaximizeButton() {
    const maximizeButton = document.querySelector('a[aria-label="window-Maximize"]');
    if (maximizeButton) {
        maximizeButton.click();
        console.log('Клик по кнопке Maximize выполнен.');
    } else {
        console.log('Кнопка Maximize не найдена.');
    }
  }


  // Отслеживание состояния loading-mask и перезагрузка страницы, если не исчезает
  function monitorLoadingMaskInIframe() {
    const iframe = document.querySelector('.k-content-frame');
    if (!iframe) {
    ...
    iframe.addEventListener('load', () => {
            const captchaIframe = document.querySelector('iframe[title="Verify Registration"]');
            console.log(captchaIframe)

            if (captchaIframe && IS_SUBMIT_CLICKED) {
                analyzeAndSelectCaptchaImages(captchaIframe);
            }
    });
    ...
    try {
        ...
        const timeout = setTimeout(() => {
            console.log('Loading mask не исчез в течение 10 секунд. Перезагрузка страницы.');
            location.reload(); // Перезагружаем всю страницу
        }, 6000);

        const interval = setInterval(() => {
            const loadingMask = iframeDocument.querySelector('.k-loading-mask');
            if (!loadingMask) {
                console.log('Loading mask исчез. Перезагрузка не требуется.');
  ...
  function findVisibleDivLikeDevAbilities() {
    const iframe = document.querySelector('.k-window-content iframe');
    if (!iframe) {
        return;
    }
    ...
    try {
        const iframeDocument = iframe.contentDocument || iframe.contentWindow.document;
        const divs = iframeDocument.querySelectorAll('div[class^="col-12 box-label"]');
        ...
        for (const div of divs) {
            ...
            const elementAtPoint = iframeDocument.elementFromPoint(centerX, centerY);
...
function isElementVisible(element, doc) {
    ...
    function filterAndRemoveUnnecessaryElements(visibleImages, groups, doc) {
        if (groups.potential.length > 0) {
            ...
            visibleImages.forEach(item => {
                ...
                const isButton = item.element.classList.contains('img-action') || item.element.closest('.img-action-div') !== null;
            ...
            for (const el of allElements) {
                ...
                const isButton = el.classList.contains('img-action') || el.closest('.img-action-div') !== null;
    ...
    function clickSubmitButton(doc) {
        ...
        const actionButtons = doc.querySelector('.img-actions').lastElementChild.click();
        console.log(result);
    }
...
function preprocessImageWithOpenCV(imageUrl, mode) {
    ...
    function analyzeAndSelectCaptchaImages(iframe, isFirstAnalyze) {
      ...
      result = [];
        try {
            ...
            const potentialImages = findAllPotentialCaptchaImages(iframeDocument);
            console.log(`Найдено ${potentialImages.length} потенциальных изображений`);

            const captchaContainer = findCaptchaContainer(iframeDocument);
            console.log('Найден контейнер капчи:', captchaContainer);

            const visibleImages = potentialImages.filter(item => {
                return captchaContainer.contains(item.element) && isElementVisible(item.element, iframeDocument);
            });
            ...
            filteredImages = filterAndRemoveUnnecessaryElements(uniqueVisibleImages, groups, iframeDocument);
            console.log(`После фильтрации осталось ${filteredImages.length} элементов`);

            if (groups.potential.length > 0) {
                groups.potential.forEach((group, index) => {
                    startAnalizeAndSelectCaptchaImages(iframeDocument, group.elements);
                });
            } else {
                alert('Не найдено потенциальных групп с ~9 элементами');

                startAnalizeAndSelectCaptchaImages(iframeDocument, filteredImages);
            }
